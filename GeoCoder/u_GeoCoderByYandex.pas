  ...
  while (CurPos<length(Vstr2Find)) and (not err)do begin
   ...
   if not ParseErr then begin // если нашли признак валидности данных
    ...
    if i>j then begin
     ...
     i:=1;
     if ALPosEx('"Categories":', Buffer, i)>i then begin// значит есть категория
      sdesc:=sdesc+' (';
      while ALPosEx('{"name":"', Buffer, i)>i do begin // названий категорий может быть несколько поэтому будем их сцеплять (банки-банкоматы)
