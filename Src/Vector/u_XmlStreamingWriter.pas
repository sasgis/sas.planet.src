...
{******************************************************************************}
procedure MoveChars(const ASrc: PChar; var ADst: PChar; const ACount: Integer); inline;
begin
  Move(ASrc^, ADst^, ACount * SizeOf(Char));
  Inc(ADst, ACount);
end;
...
begin
  ...
  for I := 1 to VLen do begin
    case VSrc^ of
      '&': Inc(VNewLen, IfThen(AUseNumericReference, 5, 5));
      '<': Inc(VNewLen, IfThen(AUseNumericReference, 5, 4));
      '>': Inc(VNewLen, IfThen(AUseNumericReference, 5, 4));
    else
      Inc(VNewLen);
    end;
...
begin
  ...
  for I := 1 to VLen do begin
    case VSrc^ of
      ...
      '<':  Inc(VNewLen, IfThen(AUseNumericReference, 5, 4));
      '>':  Inc(VNewLen, IfThen(AUseNumericReference, 5, 4));
      '"':  Inc(VNewLen, IfThen(AUseNumericReference, 5, 6));
      '''': Inc(VNewLen, IfThen(AUseNumericReference, 5, 6));
    else
      Inc(VNewLen);
    end;
